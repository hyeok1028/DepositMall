...
@Tag(name = "Admin - Member", description = "관리자 전용 회원 관리 API")
public class AdminMemberController {

    private final MemberService service;

    @GetMapping({"", "/"})
    @Operation(summary = "전체 회원 목록 조회", description = "관리자가 전체 회원의 목록을 조회합니다.")
    ...
    @PutMapping("/{id}")
    @Operation(summary = "회원 정보 수정", description = "관리자가 회원 정보를 수정합니다 (비활성화 등). 성공 시 success: true를 반환합니다.")
    public ResponseEntity<ApiResponse<MemberResponse>> editMember(@PathVariable Long id,
        ...
        return ResponseEntity.ok(ApiResponse.success(response, "Member information updated"));
    }

    @DeleteMapping("/{id}")
    @Operation(summary = "회원 강제 탈퇴", description = "관리자가 회원을 시스템에서 삭제합니다. 성공 시 success: true를 반환합니다.")
